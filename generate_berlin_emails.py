...
# Each takes (name: str, sender: str) -> str
# All emails are under 150 words, written in natural German.
# ---------------------------------------------------------------------------
...
def c3(name, sender):
    return (
        f"Hallo {name}-Team,\n\n"
        f"Ihr Café hier in Berlin hat mich beim Recherchieren angesprochen – schöne Atmosphäre!\n\n"
...
# ---------------------------------------------------------------------------
# Business data: (Name, Category, Email, Website, template_function)
# ---------------------------------------------------------------------------
...
def main():
    with open(OUTPUT_FILE, "w", newline="", encoding="utf-8-sig") as f:
        ...
        writer.writerow(["Business Name", "Category", "Email", "Website", "Email Body"])
        for name, category, email, website, tmpl_fn in businesses:
            body = tmpl_fn(name, SENDER_NAME)
            writer.writerow([name, category, email, website, body])

    print(f"✓ CSV exported → {OUTPUT_FILE}")
    print(f"  {len(businesses)} businesses written.")
    print()
    print("IMPORTANT: Search for '[DEIN NAME]' in the CSV and replace it with your actual name before sending!")
